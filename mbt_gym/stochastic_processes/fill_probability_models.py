...
class FillProbabilityModel(StochasticProcessModel):
    ...
    @abc.abstractmethod
    def _get_fill_probabilities(self, depths: np.ndarray, **args) -> np.ndarray:
    ...
    def get_fills(self, depths: np.ndarray, **args) -> np.ndarray:
        ...
        return unif < self._get_fill_probabilities(depths, args)
...
class CompetitionFillFunction(FillProbabilityModel):
    ...
    def get_fills(self, depths: np.ndarray, competition: np.ndarray) -> np.ndarray:
        ...
        unif = self.rng.uniform(size=(self.num_trajectories, 2))
        return unif < self._get_fill_probabilities(depths, competition)
